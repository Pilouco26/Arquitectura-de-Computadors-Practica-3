...
for nFile in nFiles:
    ...
    command_template = ["srun", "-p", "orca", "-c", "128", "time", "./p", str(nFile), "%s"]  # Corrected nfile to str(nFile)

    if os.path.exists("Orca" + str(nFile) + ".txt"):  # Corrected nfile to str(nFile)
        os.remove("Orca" + str(nFile) + ".txt")  # Corrected nfile to str(nFile)
    ...
    for nThread in nThreads:
        ...
        with open("Orca" + str(nFile) + ".txt", "a") as f:  # Corrected nfile to str(nFile)
            ...
            print(output_str)
            f.write("\n-------------------------------------------\n\n")
        with open("errorOrca" + str(nFile) + ".txt", "a") as e:  # Corrected nfile to str(nFile)
            if stderr:
